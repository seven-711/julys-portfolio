      ...
      <div className={`w-full transition-all duration-500 ${scrolled ? 'px-4 md:px-8' : 'px-0'}`}>
        ...
        >
          ...
          <div className={`hidden md:flex items-center space-x-2 pr-8 transition-all duration-300 ${
            ...
            <Link 
              href="/contact" 
              className="px-4 py-2 text-sm font-medium text-white bg-gradient-to-r from-[#40ffaa] to-[#4079ff] rounded-lg hover:opacity-90 transition-opacity whitespace-nowrap"
            >
              Contact Me
            </Link>
          </div>
          ...
          <div className="flex flex-col space-y-2 px-6 py-2">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                href={link.path}
                className={`px-4 py-3 rounded-lg text-sm font-medium transition-colors px-8${
                  pathname === link.path
                    ? 'text-white bg-white/10'
